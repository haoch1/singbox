...
查看节点会为每个节点显示序号，节点之间保留空行，只显示节点名称、协议、端口和 VLESS 链接，不生成聚合订阅。修改节点时直接回车保持原端口不会触发自身端口冲突提示，可以分别修改名称、客户端连接地址、监听端口、UUID、SNI 或重新生成 Reality 凭据。查看实时日志默认显示最近 20 行，按 `Ctrl+C` 返回主菜单。选择 `[6] 端口转发` 时使用 Realm 原项目的 `/usr/local/bin/r`，首次使用会自动安装该管理脚本。选择 `[14] 一键卸载` 会同时清理 sing-box 和 Realm 创建的服务、配置、核心、备份、日志、锁、临时文件和管理命令，系统预先存在的外部核心和共享依赖会保留。
