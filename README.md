...
- 管理脚本更新、实时日志和一键卸载
- Realm 风格单列交互，日志默认显示最近 100 行并可按 Ctrl+C 返回主菜单
...
查看节点只显示节点名称、协议、端口和 VLESS 链接，不生成聚合订阅。修改节点时可以分别修改名称、客户端连接地址、监听端口、UUID、SNI 或重新生成 Reality 凭据。查看实时日志时按 `Ctrl+C` 返回主菜单。
